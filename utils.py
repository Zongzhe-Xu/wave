...
class MMD_loss(nn.Module):
    ...
    def guassian_kernel(self, source, target):
        ...
        # print(source.size(), target.size()) # torch.Size([96, 5, 1024]) torch.Size([96])               # roberta [96,768],[96,768]
...
def set_grad_state(module, state, finetune_method='all'): #
    if finetune_method=='lora':
        for n, m in module.named_parameters():
            if 'lora' in n:
                m.requires_grad = True
    elif finetune_method == 'freeze':
        pass
    else:
...
def set_param_grad(model, finetune_method):
    ...
    if finetune_method == "layernorm":
        for n, m in model.named_parameters():
            ...
            if 'layer.10' in n or 'layer.11' in n or 'embedder' in n or 'predictor' in n:
                m.requires_grad = True
            elif 'layer' in n or 'orm' in n:
                if 'layernorm' in n or 'LayerNorm' in n or 'orm' in n:
                    continue
                else:
                    m.requires_grad = False
...
def embedder_init(source, target, train_embedder=False, match_stats=False, args=None):
    if train_embedder:
