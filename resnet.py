...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("shape before layer1", x.shape)
        x = self.layer1(x)
        # print("shape before layer2", x.shape)
        x = self.layer2(x)
...
def resnet20(in_channel=3, num_classes=10, ks=None, ds=None, strides=None, pool_k=1, remain_shape=False, squeeze=False, activation=None):
    return ResNet(BasicBlock, [2, 2, 2], in_channel=in_channel, num_classes=num_classes, ks=ks, ds=ds, strides=strides, pool_k=pool_k, remain_shape=remain_shape, squeeze=squeeze, activation=activation)
