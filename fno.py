...
class SimpleBlock2d(nn.Module):
    def __init__(self, modes1, modes2,  width, op='fno', ks=None, ds=None, einsum=True, padding_mode='circular', out_channels=1, input_channels=3):
        ...
        self.modes2 = modes2
        self.width = width
        self.fc0 = nn.Linear(input_channels, self.width) # input channel is 3: (a(x, y), x, y)
        ...
        # self.fc1 = nn.Conv2d(self.width, out_channels, 1)

    def forward(self, x):
        batchsize = x.shape[0]
        size_x, size_y = x.shape[1], x.shape[2]
        #print("before fc0", x.shape)
        x = self.fc0(x)
        x = x.permute(0, 3, 1, 2)
        #print("after fc0", x.shape)
        x1 = self.conv0(x)
...
class Net2d(nn.Module):
    def __init__(self, modes, width, op='fno', ks=None, ds=None, input_channels = 3, output_channels = 1, **kwargs):
        ...
        self.conv1 = SimpleBlock2d(modes, modes, width, op, ks, ds, input_channels=input_channels, out_channels=output_channels, **kwargs)
